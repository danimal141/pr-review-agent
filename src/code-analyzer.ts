...
export class CodeAnalyzer {
   ...
   * コードを分析して問題を検出
   */
  async analyzeCode(code: string, filename = "code.js"): Promise<CodeAnalysisReport> {
    ...
    try {
      ...
      const criticalIssues = agentResults.reduce(
        (sum, result) => sum + result.comments.filter((c) => c.severity === "critical").length,
        0
      );
   ...
   * コード品質分析
   */
  private async runCodeAnalysis(fileData: FileData): Promise<AnalysisResult> {
    const startTime = Date.now();
   ...
   * セキュリティ分析
   */
  private async runSecurityAnalysis(fileData: FileData): Promise<AnalysisResult> {
    const startTime = Date.now();
   ...
   * スタイル分析
   */
  private async runStyleAnalysis(fileData: FileData): Promise<AnalysisResult> {
    const startTime = Date.now();
  ...
  private parseAgentResponse(
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return (analysisResult.analysisResults || []).map(
          (item: {
            line?: number;
            description?: string;
            title?: string;
  ...
  private createErrorResult(
    ...
    return {
      ...
      comments: [
        {
          line: 1,
          message: `${agentName}分析エラー: ${error instanceof Error ? error.message : String(error)}`,
