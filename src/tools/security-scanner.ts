...
export class SecurityScannerTool {
  ...
  static scanFile(content: string, filename: string): SecurityScanResult {
    ...
    // リスクスコアを計算
    const riskScore = SecurityScannerTool.calculateRiskScore(bySeverity);
  ...
  static generateSecuritySummary(results: SecurityScanResult[]): {
    ...
    // 上位の問題を抽出
    const topIssues = SecurityScannerTool.prioritizeIssues(allIssues).slice(0, 5);
   ...
   * リスクスコアを計算（0-100）
   */
  private static calculateRiskScore(bySeverity: {
    critical: number;
    high: number;
    medium: number;
    low: number;
